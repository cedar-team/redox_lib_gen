...
from redox_parser_gen.generate import LIB_DEST_DIR
...
def update_version():
    with temp_chdir(LIB_DEST_DIR):
        ...
        # Ensure the lock file is still up-to-date
        run(["poetry", "lock", "--no-update"], check=True)

        # Set the version number in redox_parser.__version__
        run(
            "echo "
            '"'
            "# -*- coding: utf-8 -*-\n"
        ...
        # Set version number in test_redox_parser_version.py
        run(
            "echo "
            '"'
            "# -*- coding: utf-8 -*-\n"
            "# ----  AUTO-GENERATED BY redox_parser_gen. DO NOT MODIFY MANUALLY!!  ---- #\n"
            "from redox_parser import __version__\n"
            "\n"
            "\n"
            "def test_version():\n"
            '    assert __version__ == \\"`poetry version --short`\\"'
            '"'
            f" > {LIB_DEST_DIR / '..' / 'tests' / 'test_redox_parser_version.py'}",
            shell=True,
            check=True,
        )
